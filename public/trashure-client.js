...
const handlePinClick = function (e) {
  ...
  axios.get(url).then((res) => {
    res.data.forEach(function (data) {
      ...
      let fakevent = {
        location: { latitude: Number(data.lat), longitude: Number(data.long) },
      };
      ...
      axios.get(url).then((res) => {
        document.querySelector(
          '.owner-of-item'
        ).textContent = `Owner: ${res.data[0].name}`;
      });
      ...
      ).textContent = `Expiration Date: ${convertDate(data.expiration_date)}`;
      document.querySelector(
        '.pickup-date-of-item'
      ).textContent = `Pickup Date: ${convertDate(data.pickup_date)}`;
      document.querySelector(
        '.pickup-time-of-item'
      ).textContent = `Pickup Time: ${startTime} - ${endTime}`;
